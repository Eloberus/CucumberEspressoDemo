...
if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
    ...
    ${HOME}/google-cloud-sdk/bin/gcloud --quiet config set project ${PROJECT_ID}
    ...
    ${HOME}/google-cloud-sdk/bin/gcloud firebase test android run --type instrumentation --app ${APK_PATH} --test ${TEST_APK_PATH} --device model=sailfish,version=27,locale=en,orientation=portrait --timeout 30m
fi